...
def get_news_content():
    ...
    for news in news_without_text:
        ...
        try:
            if html:
                soup = BeautifulSoup(html, 'html.parser')
                article = soup.find('div', class_='post__text-html').decode_contents()  # decode_contents() - позволяет не просто получить текст находящийся внутри div, а получить html код
                if article:
                    news.text = article
                    db.session.add(news)
                    db.session.commit()
